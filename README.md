...
Double Exponential Smoothing Forecaster for the Forecasting problem category as per Ready Tensor specifications.

Double Exponential Smoothing, also known as Holt's Linear Trend Method, is a time series forecasting technique that extends Simple Exponential Smoothing to account not only for the level of the series but also for its trend.

This model is particularly useful for forecasting data where there is a trend but no seasonal pattern.
...
- A **Double Exponential Smoothing Forecaster** algorithm built using **Darts**
